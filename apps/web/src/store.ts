...
interface AppState {
  ...
  analysisProgress: {
    ...
    stages: Array<{ name: string; status: string; error?: string; result?: any }>;
    error?: string;
  } | null;
  ...
  runAnalysisStep: (step: 'policy' | 'network' | 'judge') => Promise<any>;
  evaluateRagas: () => Promise<void>;
  clearError: () => void;
...
export const useStore = create<AppState>((set, get) => ({
  ...
  runAnalysis: async () => {
    ...
    set({
      ...
      analysisSummary: null,
      analysisMetadata: null,
      analysisProgress: { overallStatus: 'running', stages: [
    ...
    try {
      ...
      const pollInterval = setInterval(async () => {
        try {
          const progress = await api.getAnalysisProgress(sessionId);
          ...
          if (progress.overallStatus === 'completed' && progress.finalResult) {
            clearInterval(pollInterval);
            set({
              findings: progress.finalResult.findings,
              analysisSummary: progress.finalResult.summary,
              analysisMetadata: progress.finalResult.agentMetadata,
              isAnalyzing: false,
              analysisProgress: null,
            });
          } else if (progress.overallStatus === 'failed') {
            clearInterval(pollInterval);
            set({ error: progress.error || 'Analysis failed', isAnalyzing: false, analysisProgress: null });
          }
          // else: still running, continue polling
        } catch (pollErr: any) {
          clearInterval(pollInterval);
          set({ error: pollErr.message, isAnalyzing: false, analysisProgress: null });
        }
      }, 2000);
      ...
      setTimeout(() => {
        ...
        if (state.isAnalyzing) {
          set({ error: 'Analysis timed out after 10 minutes', isAnalyzing: false, analysisProgress: null });
        }
      }, 600000);

    } catch (err: any) {
      set({ error: err.message, isAnalyzing: false, analysisProgress: null });
    }
  },

  runAnalysisStep: async (step: 'policy' | 'network' | 'judge') => {
    const { sessionId } = get();
    if (!sessionId) return;

    set({ isAnalyzing: true, error: null });
    try {
      const response = await api.runAnalysisStep(sessionId, step);
      const result = response?.result;

      // If judge step returned findings, update the store
      if (step === 'judge' && result?.findings) {
        set({
          findings: result.findings,
          analysisSummary: result.summary || null,
          analysisMetadata: {
            policyRulesExtracted: result.summary?.totalRules || 0,
            trafficConversationsAnalyzed: 0,
            trafficAnomaliesDetected: 0,
            processingTimeMs: 0,
          },
        });
      }
  ...
  resetFindings: () => {
    set({ findings: [], analysisSummary: null, analysisMetadata: null, ragasResult: null });
  },

  evaluateRagas: async () => {
    const { sessionId, findings } = get();
    if (!sessionId) {
      set({ error: 'No active session' });
      return;
    }
    if (findings.length === 0) {
      set({ error: 'No findings to evaluate. Run analysis first.' });
      return;
    }
    set({ isEvaluatingRagas: true, error: null });
    try {
      const result = await api.evaluateRagas(sessionId);
      set({ ragasResult: result, isEvaluatingRagas: false });
    } catch (err: any) {
      set({ error: err.message, isEvaluatingRagas: false });
    }
  },
}));
