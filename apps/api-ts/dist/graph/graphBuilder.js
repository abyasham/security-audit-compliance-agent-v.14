...
class GraphBuilder {
     ...
     */
    parseConversations(raw, store, protocol) {
        ...
        for (const line of lines) {
            ...
            if (!inTable)
                continue;
            if (line.trim() === '' || line.includes('Frames') || line.includes('Relative'))
                continue;
    ...
    attachProtocols(store, protoHierarchy) {
        const lines = protoHierarchy.split('\n').map(l => l.replace(/\r$/, ''));
        for (const line of lines) {
            if (!line.includes('Protocol') && !line.includes('├─') && !line.includes('└─'))
                continue;
    ...
    async addSamplePackets(filePath, store, count) {
        const batchSize = 100;
    ...
    findStreamForPacket(store, srcIP, dstIP, protocol) {
