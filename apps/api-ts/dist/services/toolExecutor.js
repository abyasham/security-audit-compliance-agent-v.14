...
class ToolExecutor {
     ...
     */
    resolveCaptureFile(args, sessionId) {
        const session = this.store.getSession(sessionId);
        if (!session || session.captureFiles.length === 0)
            return undefined;
        const id = args.captureFileId || args.captureFile || args.file;
        if (!id)
            return session.captureFiles[0]?.filePath;
        // Try matching by ID
        let cf = session.captureFiles.find(c => c.id === id);
        // Try by filename
        if (!cf)
            cf = session.captureFiles.find(c => c.name === id);
        // Try by path suffix
        if (!cf)
            cf = session.captureFiles.find(c => c.filePath.endsWith(id));
        // Fallback
        if (!cf)
            cf = session.captureFiles[0];
        return cf?.filePath;
    }
    /**
            ...
            switch (name) {
                ...
                case 'getViolations': {
                    ...
                    if (findings.length === 0) {
                        ...
                        result = findings.map(f => `[${f.severity.toUpperCase()}] ${f.ruleId}: ${f.description}\n  Evidence: packets ${f.evidencePacketNumbers.join(', ')}`).join('\n\n');
                    }
                    break;
                }
                case 'queryGraph': {
